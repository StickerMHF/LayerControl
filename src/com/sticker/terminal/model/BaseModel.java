...
public abstract class BaseModel<M extends Model<M>> extends Model<M> {
	...
	public boolean save() {
		...
		Table t = getTable();
		this.set(t.getPrimaryKey(), SequenceRuleModel.dao.getNextVal(
				t.getName(), t.getPrimaryKey())); // 设置主键值
		return super.save();
	}
	// /**
	// * 重写update方法
	...
	// // 1.数据是否还存在
	// String sql = new
	// StringBuffer("select version from ").append(name).append(" where ").append(pk).append(" = ? ").toString();
	...
	// try {
	// Field field =
	// this.getClass().getSuperclass().getSuperclass().getDeclaredField("modifyFlag");
